...
public class OrderController {
  ...
  public String create(
    ...
    List<ContentsDTO> list = new ArrayList<ContentsDTO>(); //상품 담을 곳
    ContentsDTO dto = cservice.detail(contentsno);
    list.add(dto);
    
    model.addAttribute("list", list); //상품목록
    model.addAttribute("mdto", mdto); //회원정보
    model.addAttribute("qty", qty); //수량(orderdetail 테이블에 저장, 총금액합계)
    model.addAttribute("size", size); //size(orderdetail 테이블에 저장)
    model.addAttribute("str", "order"); //문자열 값에 따라 처리되는 값이 다르기 때문에.. 상품보기에서 주문을 한 것
    model.addAttribute("contentsno", contentsno); //단품상품번호.. 만약에 상품번호로 왔으면 많아짐. 
  ...
  public String create(
    ...
    model.addAttribute("list", list); //상품목록
    model.addAttribute("mdto", mdto); //회원정보
    model.addAttribute("qtys", qty); //수량들(orderdetail 테이블에 저장, 총금액합계)
